...
const initialMembers: Member[] = [
  { id: 1, name: "Abdullah Hassan", email: "abdullah@example.com", phone: "(555) 201-0001", role: "Imam", joinDate: "2018-03-01", status: "Active" },
  { id: 2, name: "Ahmed Ali", email: "ahmed@example.com", phone: "(555) 201-0002", role: "Board Member", joinDate: "2019-06-15", status: "Active" },
  { id: 3, name: "Sarah Khan", email: "sarah@example.com", phone: "(555) 201-0003", role: "Teacher", joinDate: "2020-01-10", status: "Active" },
  { id: 4, name: "Omar Mahmoud", email: "omar@example.com", phone: "(555) 201-0004", role: "Volunteer", joinDate: "2021-08-22", status: "Active" },
  { id: 5, name: "Fatima Hassan", email: "fatima@example.com", phone: "(555) 201-0005", role: "Member", joinDate: "2022-03-05", status: "Active" },
  { id: 6, name: "Ibrahim Yusuf", email: "ibrahim@example.com", phone: "(555) 201-0006", role: "Youth Leader", joinDate: "2021-11-30", status: "Active" },
  { id: 7, name: "Aisha Rahman", email: "aisha@example.com", phone: "(555) 201-0007", role: "Volunteer", joinDate: "2023-02-14", status: "Active" },
  { id: 8, name: "Khalid Siddiqui", email: "khalid@example.com", phone: "(555) 201-0008", role: "Member", joinDate: "2023-07-01", status: "Inactive" },
  { id: 9, name: "Maryam Noor", email: "maryam@example.com", phone: "(555) 201-0009", role: "Member", joinDate: "2024-01-20", status: "Active" },
  { id: 10, name: "Usman Farooq", email: "usman@example.com", phone: "(555) 201-0010", role: "Board Member", joinDate: "2019-09-11", status: "Active" },
];

const blank: Omit<Member, "id"> = {
  name: "", email: "", phone: "", role: ROLES[0], joinDate: "", status: "Active",
};
...
export default function CommunityAdmin() {
  ...
  function openEdit(m: Member) {
    setEditing(m);
    setForm({ name: m.name, email: m.email, phone: m.phone, role: m.role, joinDate: m.joinDate, status: m.status });
    setShowForm(true);
  }
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex flex-wrap items-center gap-4">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full border border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a5c38]/20 focus:border-[#1a5c38]"
          />
        </div>
          ...
          {["All", ...ROLES].map((r) => (
            <button key={r} onClick={() => setRoleFilter(r)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                roleFilter === r ? "bg-[#1a5c38] text-white" : "bg-gray-50 border border-gray-200 text-gray-600 hover:bg-gray-100"
              }`}>
              {r}
            </button>
          ))}
                ...
                <tr key={m.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      m.role === "Imam" || m.role === "Board Member"
                        ? "bg-purple-50 text-purple-700"
                        : m.role === "Teacher" || m.role === "Youth Leader"
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                      m.status === "Active" ? "bg-emerald-50 text-[#1a5c38]" : "bg-gray-100 text-gray-500"
                    }`}>
                      {m.status}
                    </span>
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(m)} className="text-xs text-[#1a5c38] hover:underline font-medium">Edit</button>
                      <button onClick={() => handleDelete(m.id)} className="text-xs text-red-500 hover:underline font-medium">Remove</button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as Member["status"] }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a5c38]/20 focus:border-[#1a5c38]">
