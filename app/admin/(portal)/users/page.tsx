...
import {
  subscribeAdminUsers, createAdminUser, updateAdminUser, deleteAdminUser,
  ADMIN_ROLES,
  type AdminUser, type SectionKey, type Permission, type SectionPerms,
} from "@/lib/db";
...
const blankForm = {
  ...
  status: "Active" as const, permissions: ADMIN_ROLES[ROLES[0]],
};
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            ...
            {PERMS.map((p) => (
              <th key={p} className="text-center px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-20">
                {p}
              </th>
            ))}
          </tr>
...
function PermSummary({ permissions }: { permissions: SectionPerms }) {
  ...
  if (granted.length === 0) return <span className="text-xs text-gray-400">No access</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {granted.map((s) => (
        <span key={s.key} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 font-medium">
          {s.label}
        </span>
      ))}
    </div>
...
export default function UsersAdmin() {
  ...
  function handleRoleChange(role: string) {
    setForm((f) => ({ ...f, role, permissions: ADMIN_ROLES[role] ?? emptyPerms() }));
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (code === "auth/email-already-in-use") setSaveError("This email is already registered in Firebase Auth.");
  ...
  async function handleDelete(u: AdminUser) {
    if (!confirm(`Remove ${u.name} from admin users? Their Firebase Auth account remains active.`)) return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage admin accounts and their section permissions.</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Super Admins",  value: users.filter((u) => u.role === "Super Admin").length },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Section Access</th>
                ...
                {filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-5 py-3.5 max-w-xs">
                      <PermSummary permissions={u.permissions ?? emptyPerms()} />
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                        u.status === "Active" ? "bg-emerald-50 text-emerald-primary" : "bg-gray-100 text-gray-500"
                      }`}>
                        {u.status}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button onClick={() => openEdit(u)} className="text-xs text-emerald-primary hover:underline font-medium">Edit</button>
                        <button onClick={() => handleDelete(u)} className="text-xs text-red-500 hover:underline font-medium">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-medium text-gray-600">Section Permissions</label>
                  <div className="flex gap-2">
                    {PERMS.map((p) => (
                      <span key={p} className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${PERM_COLORS[p]}`}>{p}</span>
                    ))}
                  </div>
                </div>
                <PermMatrix
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                <h2 className="font-semibold text-gray-900">{viewPerms.name}</h2>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${ROLE_COLORS[viewPerms.role] ?? "bg-gray-100 text-gray-600"}`}>
                    {viewPerms.role}
                  </span>
                  <span className="text-xs text-gray-400">{viewPerms.email}</span>
                </div>
              </div>
              <button onClick={() => setViewPerms(null)} className="text-gray-400 hover:text-gray-600">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <div className="p-6">
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {SECTIONS.map((s) => {
                      const granted = (viewPerms.permissions ?? emptyPerms())[s.key] ?? [];
                      const hasAny  = granted.length > 0;
                      return (
                        <tr key={s.key} className={hasAny ? "" : "opacity-40"}>
                          <td className="px-4 py-2.5 font-medium text-gray-700 text-sm">{s.label}</td>
                          {PERMS.map((perm) => (
                            <td key={perm} className="text-center px-3 py-2.5">
                              {granted.includes(perm) ? (
                                <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-semibold ${PERM_COLORS[perm]}`}>
                                  ✓
                                </span>
                              ) : (
            ...
            <div className="px-6 pb-5">
              <button onClick={() => { setViewPerms(null); openEdit(viewPerms); }}
